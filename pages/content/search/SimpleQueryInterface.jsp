...
function onAttributeChange(element,opComboName,txtFieldID)
{
	...
	else
	{
		...
		{
			opCombo.options[0] = new Option("Začíná znaky","<%=Operator.STARTS_WITH%>");
			opCombo.options[1] = new Option("Končí znaky","<%=Operator.ENDS_WITH%>");
			opCombo.options[2] = new Option("Obsahuje","<%=Operator.CONTAINS%>");
			opCombo.options[3] = new Option("Rovná se","<%=Operator.EQUAL%>");
			opCombo.options[4] = new Option("Nerovná se","<%=Operator.NOT_EQUALS%>");
		}
		else if (columnValue.match("<%=Constants.FIELD_TYPE_TINY_INT%>") == "<%=Constants.FIELD_TYPE_TINY_INT%>")
		{
			opCombo.options[0] = new Option("Rovná se","<%=Operator.EQUAL%>");
			opCombo.options[1] = new Option("Nerovná se","<%=Operator.NOT_EQUALS%>");
		}
		else
		{
			opCombo.options[0] = new Option("Rovná se","<%=Operator.EQUAL%>");
			opCombo.options[1] = new Option("Nerovná se","<%=Operator.NOT_EQUALS%>");
			opCombo.options[2] = new Option("Menší než","<%=Operator.LESS_THAN%>");
			opCombo.options[3] = new Option("Menší nebo roven","<%=Operator.LESS_THAN_OR_EQUALS%>");
			opCombo.options[4] = new Option("Větší než","<%=Operator.GREATER_THAN%>");
			opCombo.options[5] = new Option("Větší nebo roven","<%=Operator.GREATER_THAN_OR_EQUALS%>");
		}

		opCombo.options[opCombo.options.length] = new Option("je NULL","<%=Operator.IS_NULL%>");
		opCombo.options[opCombo.options.length] = new Option("není NULL","<%=Operator.IS_NOT_NULL%>");
	}
}
      ...
      <tr>
        <td align="right"><table width="100%" border="0" cellspacing="0" cellpadding="2">
            <tr>
              ...
              <td width="6%" align="right" nowrap="nowrap"><span class="link"><html:link href="#" onclick="<%=configAction%>" styleClass="view">Definovat zobrazení</html:link></span></td>
            </tr>
        </table></td>
...
<logic:equal name="pageOf" value="<%=Constants.PAGE_OF_SIMPLE_QUERY_INTERFACE%>">
			...
			<logic:equal name="pageOf" value="<%=Constants.PAGE_OF_SIMPLE_QUERY_INTERFACE%>">
              ...
              <td nowrap class="black_new">
								<html:select property="<%=attributeCondition%>"
											styleClass="formFieldSized10" styleId="<%=operatorId%>"
											...
											<html:option value="<%=Operator.EQUAL%>">Rovná se</html:option>
											<html:option value="<%=Operator.NOT_EQUALS%>">Nerovná se</html:option>
											<%
 ...
 %>
			  ...
			  <logic:equal name="pageOf" value="<%=Constants.PAGE_OF_SIMPLE_QUERY_INTERFACE%>">
              <td nowrap class="black_ar"><html:select
												property="<%=nextOperator%>" styleClass="formFieldSized10">
