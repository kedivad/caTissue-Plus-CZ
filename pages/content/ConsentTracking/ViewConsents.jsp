...
function checkDisable(){
	...
	for(var cnt = 0;cnt<comboData.length;cnt++){
		...
		if(selectedField.getSelectedValue()=='Odstoupeno'){
			//alert('Odstoupeno');
			...
			<tr>
				<td>
					...
					<div class="alert alert-success" id="success" style="display:none">
					   Souhlasy úspěšně aktualizovány.
					</div>
				</td>
		   </tr>
				<tr><td></td></tr>
								...
								<tr>
									...
									<logic:equal name="consentLevel" value="participant">
										...
										<script>
										  ...
										  if('${consentTierDTO.participantResponses}'=='Odstoupeno'){
									...
									<script>
										  ...
										  statusId.push("${consentTierDTO.id}");
										   if('${consentTierDTO.status}'=='Odstoupeno'){
											isConsentResponseWithdrawn = true;
										  }
				...
				<tr>
					<td align="left" colspan="2" class="buttonbg">
						<div style="display:none;" id="disableConsentCheckboxDiv"><input type='checkbox' name="disableConsentCheckbox"  style="vertical-align: middle" ><span style="vertical-align:middle" class="black_ar">Zakázat vzorky?</span>
						</div>
					</td>
				</tr>
				<tr>
		...
		<script>
			...
			function onSubmit() {
				...
				for (var cnt = 0; cnt < comboData.length; cnt++) {
					...
					jsonObj.participantResponses = selectedField
							.getSelectedText();
					jsonObj.consentStatusId = 0;
					tabDataJSON.push(jsonObj);
				}
				//consentDto.consentTierList = JSON.stringify(tabDataJSON);
				consentDto.consentLevel = document
						.getElementById("consentLevel").value;
				consentDto.consentLevelId = document
						.getElementById("consentLevelId").value;

				var consentLevelId = document.getElementById("consentLevelId").value;
				var disposeSpecimen = document
						.getElementsByName("disableConsentCheckbox")[0].checked;
				var lodder = dhtmlxAjax.postSync("CatissueCommonAjaxAction.do",
						"type=updateConsentTierStatus&consentLevel="
								+ consentLevel + "&consentLevelId="
								+ consentLevelId + "&dataJSON="
								+ JSON.stringify(tabDataJSON)
								+ "&disposeSpecimen=" + disposeSpecimen
								+ "&consentDto=" + JSON.stringify(consentDto));
				if (lodder.xmlDoc.responseText != null) {
					...
					if (response.success == "success") {
						...
						if (consentLevel == "specimen") {
							...
							nodeId = "SpecimenCollectionGroup_"
									+ consentLevelId;
							refreshTree(null, null, null, null, nodeId);
						}
