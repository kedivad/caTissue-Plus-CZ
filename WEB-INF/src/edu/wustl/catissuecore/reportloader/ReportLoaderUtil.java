...
public final class ReportLoaderUtil
{
	...
	public static String getValidSSN(String ssn)
	{
		...
		try
		{
			re = Pattern.compile("[0-9]{6}/[0-9]{4}", Pattern.CASE_INSENSITIVE);
			mat = re.matcher(ssn);
			result = mat.matches();
			if (!result)
			{
				re = Pattern.compile("[0-9]{10}", Pattern.CASE_INSENSITIVE);
				mat = re.matcher(ssn);
				result = mat.matches();
				if (result)
				{
					buff = new StringBuffer();
					buff.append(ssn.substring(0, 6)).append("/").append(ssn.substring(6, 10));
				}
			}
			else
			{
