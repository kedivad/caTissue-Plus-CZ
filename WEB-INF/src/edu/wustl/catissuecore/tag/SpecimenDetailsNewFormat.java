...
public class SpecimenDetailsNewFormat extends TagSupport
{
	...
	/** The data list types. */
	private transient final String dataListTypes[] = {"Základní", "Aliquot", "Derived"};

	/** The Constant COLUMN_NAMES. */
	public static final String[] COLUMN_NAMES = {"Základní", "Label", "Barcode", "Type", "Quantity",
	...
	public static final String[] HDR1_COLS = {"Základní", "Label", "Barcode", "Type", "Quantity",
			"Concentration", "Location", "Collected", "PrintLabel"};

	/** The Constant HDR2_COLS. */
	public static final String[] HDR2_COLS = {"Type", "Pathological Status", "Strana odběru",
			"Tissue Site"};
...
//		if(!edu.wustl.catissuecore.util.global.Variables.isSpecimenLabelGeneratorAvl
		...
		else
		{
			...
			{
				stringBuffer.append("<span class='black_ar'>Vygenerováno</span>");
			}
			else if(this.generateLabel && !Validator.isEmpty(this.displayName) && nameValue[0].contains("displayName") && nameValue[1].equals("Vygenerováno"))
			{
				stringBuffer.append("<span class='black_ar'>Vygenerováno</span>");
			}
			...
			else if(this.generateLabel && nameValue[0].contains("parentName") && !Validator.isEmpty(nameValue[1]) && nameValue[1].equals("Vygenerováno"))
			{
...
//						}
//						else if(!Validator.isEmpty(this.displayName) && this.displayName.equals("Vygenerováno") && this.generateLabel && this.columnHeaderList
//								.get(cnt).equals("specimen.label"))
//						{
//							stringBuffer.append(TD_1HLF + this.cWd + TD_2HLF);
//						}
						if(cnt == 0 && !Validator.isEmpty(this.parentName) && this.parentName.equals("Vygenerováno") && this.generateLabel && this.columnHeaderList
...
//						}
	...
	private String[] get1EleDetAt(int counter, GenericSpecimen specimen, String elementNamePrefix) throws NumberFormatException, ApplicationException
	{
				...
				.toString()))
		{
			...
			if("Vygenerováno".equals(specimen.getParentName()))
			{
			...
			String[] nameValue, GenericSpecimen specimen)
	{
		...
		stringBuffer.append(TD_OPEN);
		String hiddenLineage = this.dataListType.equalsIgnoreCase("Základní")?"specimen":lineage;
		stringBuffer.append(
