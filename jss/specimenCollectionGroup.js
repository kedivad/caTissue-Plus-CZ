...
function initializeSCGCombo(operation)
{
		...
		scgCombo.activityStatusCombo.deleteOption('-- Vybrat --');
...
function onBlurFunc() {alert(clinicalDiagnosisCombo.getLastSelectedValue());
	...
	{
		clinicalDiagnosisCombo.setComboValue("Nespecifikováno");
	}
    return true;
}*/
...
function checkForChanges()
{
	...
	if((collectionEventdateOfEvent != collectionEventdateOfEventForm)
	  ...
	  {
		var appResources = "Vámi zadaná data ohledně Sběru a Přijetí vzorků budou přeneseny na všechny vzorky pod touto Kolekcí sesbíraných vzorků (KSV) a přepíší všechny existující údaje. Chcete pokračovat?";
...
function loadSCGTabbar()
{
	...
	if("edit" == operation)
	{
		scgTabbar.addTab("editSCGtab",'<span style="font-size:13px"> Upravit KSV </span>', "150px");
		scgTabbar.addTab("reportsTab",'<span style="font-size:13px"> Report </span>', "150px");
		scgTabbar.addTab("annotationTab",'<span style="font-size:13px"> Anotace </span>',"150px");
	...
	if("add" == operation)
	{
		scgTabbar.addTab("editSCGtab",'<span style="font-size:13px"> Přidat KSV </span>', "150px");
