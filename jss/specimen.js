...
{
	var agree=confirm("Jste si jisti smazáním této položky?");
	if(agree)
	{
...
function editTag(e)
{
	...
	document.getElementById('addEditExtIdButton').value="Upravit";
...
function addEditBioHazTag(buttonElement)
{
	...
	{	var idTypeForBiohazard = biohazardCombo.getSelectedValue().split("_");
		...
		if(buttonElement.value=="Upravit")
		{
...
function editBiohazardTag(e)
{
	...
	document.getElementById('addEditBioHazButton').value="Upravit";
	biohazardTypeCombo.unSelectOption();
...
var defaultTextForExtIdName = "Zadejte název identifikátoru";
var defaultTextForExtIdValue = "Zadejte hodnotu identifikátoru";
var defaultTextForBioName = "Vyberte název biologockého nebezpečí";
var defaultTextForBioType = "Vyberte typ biologockého nebezpečí";
var biohazardCombo;
...
function onSpecimenTypeChange(selectedElement)
{
	...
	{
		case 'Buňka' : typeCombo.addOption(cellTypeOptions); break;
		case 'Molekula' : typeCombo.addOption(molecularTypeOptions); break;
		case 'Tkáň' : typeCombo.addOption(tissueTypeOptions); break;
		case 'Kapalina' : typeCombo.addOption(fluidTypeOptions); break;
		default : typeCombo.addOption(-1,"-- Vybrat --");
	}
...
function forwardToChildSpecimen(operation) {
	...
	{
		showErrorMessage("Nelze vytvořit podřazený vzorek (vzorky) ze vzorku, který není k dispozici.");
		return;
	}
...
{
	var subTypeData1 = "Zmražená Tkáň na sklíčku";
	var subTypeData2 = "Blok fixní tkáně";
	var subTypeData3 = "Blok zmražené tkáně";
	var subTypeData4 = "Nespecifikováno";
	var subTypeData5 = "Mikrodisekce";
	var subTypeData6 = "Fixní Tkáň na sklíčku";
	...
	var selectedOption = typeCombo.getComboText();
		...
		else if(className == "Tkáň" && (selectedOption == subTypeData1 || selectedOption == subTypeData2 || selectedOption == subTypeData3 || selectedOption == subTypeData4 || selectedOption == subTypeData6))
		{
...
function processData(obj)
{
	...
	else if(obj.name=='quantity' && (obj.value.trim()=="" || !isNumeric(obj.value.trim()) || Number(obj.value) < Number(document.getElementById('availableQuantity').value)))
	{
		...
		if(!isNumeric(obj.value.trim()))
		{
			...
			errorDiv.innerHTML = "(Zadejte platné číslo)";
		}
		else if(obj.value.trim()!="" && Number(obj.value) < Number(availableQuantityElement.value))
		{
			...
			avlErrorDiv.innerHTML = "(Nemůže být větší než počáteční množství)";
	...
	else if(obj.name=='availableQuantity' && (!isNumeric(obj.value.trim()) || Number(obj.value) > Number(document.getElementById('quantity').value)))
	{
		...
		if(!isNumeric(obj.value))
			...
			errorDiv.innerHTML = "(Nemůže být větší než počáteční množství)";
...
function validateAndProcessComboData(obj)
{
	...
	if(obj.getSelectedValue()=='-1' || obj.getSelectedText()=='-- Vybrat --' || obj.getSelectedText().trim()=="")
	{
	...
	else
	{
		...
		if(obj.name=='className' && typeCombo.getComboText()=='-- Vybrat --')
		{
...
function submitTabData(operation)
{
	...
	{
		showErrorMessage("Nelze odeslat. Prosím vyřešte zvýrazněné problémy");
		scrollToTop();
	}
}
...
*/
		...
		specimenTabbar.setLabel("specimenDetailsTab",'<span style="font-size:13px"> Podrobnosti vzorku </span>', "150px");	
		specimenTabbar.addTab("eventsTab",'<span style="font-size:13px"> Události </span>', "150px");
	if(reportId != null && reportId != -1  && reportId != -2)
	{
		specimenTabbar.addTab("reportsTab",'<span style="font-size:13px"> Zobrazit report(y)</span>',"150px");
	}
	specimenTabbar.addTab("annotationTab",'<span style="font-size:13px">Zobrazit anotace </span>',"150px");
	/*if(hasConsents){
